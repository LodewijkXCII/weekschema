    ...
    <div v-if="recipe" class="fixed inset-0 z-50 flex justify-end bg-foreground/40 backdrop-blur-[2px]" @click="$emit('close')">
      <aside class="relative flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-border bg-card p-5 shadow-lift" @click.stop>
        ...
        <div class="mt-3 flex items-start justify-between gap-2">
          ...
          <button
            type="button"
            class="grid size-9 shrink-0 place-items-center rounded-lg border border-border text-muted-foreground transition-colors"
        ...
        <div v-if="recipe.tags?.length" class="mb-2 flex flex-wrap gap-1">
          <span v-for="t in recipe.tags" :key="t" class="rounded-full bg-muted px-2 py-0.5 text-[11px] text-muted-foreground">#{{ t }}</span>
        </div>
        ...
        <div class="mb-3 flex gap-2">
          <button
            ...
            :class="recipe.mijnReactie === 1 ? 'border-transparent bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground hover:border-primary'"
            @click="react(1)"
          ><ThumbsUp class="size-3.5" /> {{ recipe.duimpjesOmhoog ?? 0 }}</button>
          <button
            type="button"
            class="inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-colors"
            :class="recipe.mijnReactie === -1 ? 'border-transparent bg-primary text-primary-foreground' : 'border-border bg-background text-muted-foreground hover:border-primary'"
            @click="react(-1)"
          ><ThumbsDown class="size-3.5" /> {{ recipe.duimpjesOmlaag ?? 0 }}</button>
        </div>
        ...
        <ul class="space-y-1 pl-1 text-sm">
          <li v-for="ri in recipe.ingredients" :key="ri.id ?? ri.ingredient.id" class="flex justify-between">
            ...
            <span class="tabular-nums text-muted-foreground">{{ Math.round(ri.hoeveelheidGram * schaal) }}g</span>
          </li>
        </ul>
...
import { X, Camera, Star, TriangleAlert, ThumbsUp, ThumbsDown, Minus, Plus } from "lucide-vue-next";
...
const perPortie = computed(() => {
  const totals = { kcal: 0, eiwit: 0, vet: 0, kh: 0 };
  if (!props.recipe) return totals;
  for (const ri of props.recipe.ingredients ?? []) {
  ...
  const porties = props.recipe.porties || 1;
